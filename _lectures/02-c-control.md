---
title: "02 Conditionals, Loops, and Arrays"
---

[*Reading: Dive into Systems §1.3–1.5*](https://diveintosystems.org/book/C1-C_intro/conditionals.html)

Building on last time, we move from C's basic syntax into control structures (conditionals, loops, and functions) and then cover the most common compound data type, the array.

## Conditionals

As seen below, `if`-`else` syntax in C and Python is almost identical. Like we discussed with the `main` function, Python uses indentation to indicate the body of a conditional, while C uses curly braces `{}`.
...
**Relational operators take operand(s) of the same type and evaluate to zero (false) or nonzero (true):** 
- equality `==`
- inequality `!=` 
- comparison `<`, `<=`, `>`, `>=`.

**logical operators take integer "Boolean" operand(s) and evaluate to either zero (false) or nonzero (true)** 
- negation `!`, 
- and `&&`, 
- or `||`. 

`&&` and `||` are *short-circuiting* — `&&` stops evaluating as soon as it hits a false operand, `||` stops as soon as it hits a true one, so the remaining operand(s) aren't evaluated at all.
...
C supports `for` and `while` loops like Python, plus `do`-`while`.

**`for` loops:** differ significantly in C and Python (C `for` loops are much closer to Java). Python's `for` iterates over a sequence; C's `for` is a general-purpose loop built from three parts:
...
- Python's `len()` tells you how many items are in a list; C has no equivalent, so we keep a `size` variable to track it. 
- In C, array elements are laid out in consecutive locations in memory. Python hides how its lists are stored.

**Array access.** C only supports accessing elements by indexing (Python has slices and other tricks):
...
The compiler is happy to compile this. Running it can crash your program, silently corrupt some other variable's value, or appear to do nothing at all — and the behavior can differ from run to run.

## Strings

C has no string type. Recall that an array can be declared larger than what a program actually uses — which means a program can't assume a string's length equals the size of the array holding it. So C strings end with the special **null character** (`'\0'`), marking where the string ends. A string ending in `'\0'` is called *null-terminated*.
